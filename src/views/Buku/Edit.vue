<template>
    <div>
        ...
        <ButtonSubmit @click="actionEditBook()" buttonText="Ubah" />
    </div>
</template>
<script>
...
export default {
    ...
    methods: {
        ...
        async actionEditBook() {
            this.axios.post(`${this.urlApi}book/${this.idParam}/update`, {
                ...this.form,
            }, {
                'headers': {
        ...
        async getBookData() {
            this.axios.get('http://perpus-api.mamorasoft.com/api/book/' + this.idParam, { 'headers': { 'Authorization': 'Bearer ' + localStorage.getItem('_token') } }).then(res => {
                this.form = res.data.data.book;
                this.imgSrc = 'http://perpus-api.mamorasoft.com/storage/' + res.data.data.book.path;
            }).catch((err) => {
                console.log(err)
            })
        },
